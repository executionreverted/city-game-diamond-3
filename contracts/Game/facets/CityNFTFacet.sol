...
contract CityNFTFacet is Modifiers {
    ...
    function balanceOf(address _owner) external view returns (uint256 balance_) {
        require(_owner != address(0), "CityNFTFacet: _owner can't be address(0)");
    ...
    function tokenByIndex(uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.tokenIds.length, "CityNFTFacet: index beyond supply");
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.ownerTokenIds[_owner].length, "CityNFTFacet: index beyond owner balance");
    ...
    function batchOwnerOf(uint256[] calldata _tokenIds) external view returns (address[] memory owners_) {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            owners_[i] = s.CityList[_tokenIds[i]].Operator;
            require(owners_[i] != address(0), "CityNFTFacet: invalid _tokenId");
        }
    }
    ...
    function internalTransferFrom(address _sender, address _from, address _to, uint256 _tokenId) internal {
        require(_to != address(0), "CityNFTFacet: Can't transfer to 0 address");
        require(_from != address(0), "CityNFTFacet: _from can't be 0 address");
        require(_from == s.CityList[_tokenId].Operator, "CityNFTFacet: _from is not owner, transfer failed");
        require(
            _sender == _from || s.operators[_from][_sender] || _sender == s.approved[_tokenId],
            "CityNFTFacet: Not owner or approved to transfer"
        );
        LibCities.transfer(_from, _to, _tokenId);
    ...
    function approve(address _approved, uint256 _tokenId) external {
        address owner = s.CityList[_tokenId].Operator;
